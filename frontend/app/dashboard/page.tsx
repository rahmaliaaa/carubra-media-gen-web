...
export default function DashboardPage() {
  ...
  const scheduledLabel = scheduledPosts.length > 0 ? `${scheduledPosts.length} konten sudah dijadwalkan` : "Belum ada konten terjadwalkan"
  const connectionLabel = connections.length > 0 ? `${connections.length} akun sudah terhubung` : "Belum ada akun sosial yang terhubung"
        ...
        <div className="space-y-4">
                  ...
                  {upcoming.map((post) => (
                    <div key={post._id} className="rounded-3xl border border-white/10 bg-slate-950/70 p-4">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <p className="font-semibold text-white line-clamp-2">{post.caption || "Konten terjadwal"}</p>
          ...
          {fetchError ? (
            <Card className="bg-red-950/80 border border-red-700/40">
              <CardContent>
                <p className="text-sm text-red-200">Terjadi kesalahan saat memuat data: {fetchError}</p>
              </CardContent>
            </Card>
          ) : null}
        </div>
            ...
            <CardContent className="space-y-4">
              <div className="rounded-3xl border border-white/10 bg-slate-950/70 p-4">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className="rounded-full bg-emerald-500/10 px-3 py-1 text-xs text-emerald-200">{connections.length}</span>
                </div>
          ...
          <Card className="bg-card">
            ...
            <CardContent>
              <div className="space-y-3 text-sm text-slate-300">
                <p>Pastikan deskripsi, hashtag, dan tipe postingan sudah sesuai sebelum jadwal dijalankan.</p>
                <p>Ingin lebih cepat? Gunakan fitur Image AI atau Video AI untuk konten berkualitas dengan cepat.</p>
